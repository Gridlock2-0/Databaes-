...
add_all_encodings(tr_feat, y_tr, val_feat, test_feat=None)
...
----------------------
BASE_COLS       26  (24 prev + tod_bucket + prefix4_le)
...
LOO_TE_SPECS = [
    ...
    (['prefix5', 'hour'],        'te_p5_hr'),     # neighbourhood × hour rhythm
    (['prefix5', 'tod_bucket'],  'te_p5_todb'),   # neighbourhood × time-of-day bucket
    (['NumberofLanes', 'hour'],  'te_nl_hr'),     # road capacity × hour
    (['LargeVehicles', 'hour'],  'te_lv_hr'),     # heavy traffic type × hour
]
...
NON_TE_COLS = BASE_COLS + LAG_COLS   # 35
ALL_COLS    = BASE_COLS + LAG_COLS + TE_COLS  # 54
...
def _apply_te_map(df, group_cols, te_map, fallback):
    """Apply a precomputed group->value map; fill unknowns with fallback."""
    if len(group_cols) == 1:
...
def add_all_encodings(
    ...
    Training rows → LOO mean (exact) + group median for LOO_TE_SPECS.
    Val / test   → regular group mean + group median from training fold.
    RoadType     → smoothed TE for all splits.
    """
    global_mean = float(y_tr.mean())
